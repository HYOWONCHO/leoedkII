...
efi_boot_fsbl_load(LV_t *rdlv)
{
    ...
    ret = SBC_GetFileSize(L"\\EFI\\BOOT\\bootx64.efi", (UINTN *)&rdlv->length);
    if (ret != SBCOK) {
      goto errdone;
    }
    ...
    ret = SBC_FindFileBufHndl(L"\\EFI\\BOOT\\bootx64.efi", &HandleCount, hndl);
    SBC_RET_VALIDATE_ERRCODEMSG((ret == SBCOK), ret, "File not found");

    retval = SBC_ReadFile(hndl[HandleCount], L"\\EFI\\BOOT\\bootx64.efi", rdlv);
    if (EFI_ERROR(retval)) {
      eprint("%s filr read fail : %r", L"\\EFI\\BOOT\\bootx64.efi", retval);
      ret = SBCIO;
      goto errdone;
    }
...
SBCStatus  _read_fsbl_image(LV_t *lv)
{
    ...
    UINT16              *fsblid  = L"\\EFI\\BOOT\\bootx64.efi";
