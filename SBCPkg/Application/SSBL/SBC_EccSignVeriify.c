...
/**
 * @file SBC_EccSignVerify.c
 * @brief Message is sign and verify using ECDSA
 *
 * @author LEON
 * @version 1.0
 * @date 2025-10-31
...
VOID SBC_EcKeyGen_Test(VOID)
{
    SBCStatus ret = SBCOK;
    at_key_t    key;

    UINT8 privkey[32] = {
    	0xF5, 0xAD, 0x5D, 0xFC, 0xE9, 0x02, 0x38, 0x55, 0x12, 0x48, 0x6E, 0xE1,
    	0x37, 0x95, 0x6C, 0x06, 0x7B, 0x84, 0x9D, 0x40, 0xF3, 0x6C, 0xB8, 0xF6,
    	0x75, 0x29, 0xD2, 0x78, 0xB9, 0x73, 0x4F, 0x1F
    };

    UINT8 pubkey[64] = {
    	0xD1, 0x9B, 0x8F, 0x5C, 0x5D, 0x01, 0x77, 0x57, 0xD8, 0xA8, 0xE3, 0x87,
    	0x03, 0xC8, 0x61, 0x31, 0xEB, 0x8E, 0x7D, 0x2F, 0xCC, 0x7D, 0x90, 0x71,
    	0xD3, 0xD6, 0x9F, 0xF6, 0x65, 0x1E, 0xAD, 0x4E, 0xA2, 0x43, 0xB1, 0x56,
    	0xBC, 0xB5, 0x66, 0x55, 0x90, 0x9D, 0x6D, 0x21, 0x42, 0xC8, 0x53, 0x9A,
    	0x1F, 0xA4, 0x85, 0x60, 0xDD, 0x6E, 0xD8, 0x4B, 0x9C, 0x86, 0x59, 0x82,
    	0x66, 0x42, 0x10, 0x60
    };

    ret = SBC_DICESeedKeyPair(privkey, &key);
    if(ret != SBCOK) {
        Print(L"SBC_DICESeedKeyPair fail \n");
        return;
    }

    Print(L"Key Length priv: %d public : %d \n", key.dl, key.ql);
    SBC_mem_print_bin("Private Key", key.d, 32);
    SBC_mem_print_bin("Public Key", key.q.value, 64);

    if(CompareMem((const void *)key.q.value, (const void *)pubkey, 64) != 0) {
        Print(L"Public key compare fail \n");
        return;
    }

    Print(L"Public Key compare Success \n");
