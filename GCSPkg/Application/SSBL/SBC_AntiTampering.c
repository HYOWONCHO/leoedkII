...
static SBCStatus sbc_set_nvme_serial(hw_uniqueinfo_t *p,
                                     const UINT8 *serial,
                                     UINTN serial_len) {
    if (p == NULL || serial == NULL || serial_len == 0) return SBCNULLP;
...
SBCStatus _nvme_get_serial(hw_uniqueinfo_t *p) {
    ...
    dprint("SSBL GCS SATA Serial number !!!"); 
...
SBCStatus  SBC_FirmwareIdKyeVerify(VOID *priv)
{
    ...
    SBC_external_mem_print_bin("FirmwareId. Priv", key_pair.d, sizeof key_pair.d);
    SBC_external_mem_print_bin("FirmwareId. Pub", key_pair.q.value, sizeof key_pair.q);
    ...
    SBC_NV_SLOT *slot = SBC_NvFindSlotByName("FirmwareID CA");
    if (slot == NULL) {
...
SBCStatus  SBC_OSIdKyeVerify(VOID *priv)
{
    ...
    SBC_external_mem_print_bin("OSID Priv", key_pair.d, sizeof key_pair.d);
    SBC_external_mem_print_bin("OSID Pub", key_pair.q.value , sizeof key_pair.q);
#ifdef _SBC_TPM_
    loadbuf = AllocateZeroPool(1024);
    SBC_RET_VALIDATE_ERRCODEMSG((loadbuf != NULL),
                                SBCNULLP,
    ...
    SBC_NV_SLOT *slot = SBC_NvFindSlotByName("OSID CA");
    if (slot == NULL) {
        eprint("TPM Slot information find fail");
        ret = SBCNULLP;
        goto errdone;
    ...
    //SBC_mem_print_bin("OSID cert", (UINT8 *)loadbuf, ldlen);
...
SBCStatus SBC_GenMigrationKey(void *priv, void *outmsg)
{
    ...
    if (len_fsbl <= 0 || len_ssbl <= 0) {

        eprint("Invalid migration key bootloader length");
        ret = SBCZEROL;
        goto errdone;
    }
    ...
    int bank = (p->prevmode == 1 && p->bm == BOOT_MODE_UPDATE)
               ? p->pvs_sw_bnk
               : p->curr_sw_bnk;

    startaddr = BOOT_SECTOR1_OFS + (BOOT_FW_IMGMAX * (bank - 1));
...
//  }
    ...
    if (fwinf->mbr.fsbln <= 0 || fwinf->mbr.ssbln <= 0) {
        ...
        eprint("Into RawFS, Invalid migration key bootloader length");
        ret = SBCZEROL;
        goto errdone;
    }
...
//  }
    ...
    // comment by leon at 260428
    //if (p->prevmode == 0 && p->bm == BOOT_MODE_RECOVERY) {
    if (p->bm == BOOT_MODE_RECOVERY) {
        // comment by leon at 20260428
            startaddr = (BOOT_SECTOR1_OFS | (BOOT_FW_IMGMAX *  (p->curr_sw_bnk - 1)));

//          int rcv_bank = (p->prevmode == 1)
//              ? p->pvs_sw_bnk
//              : p->curr_sw_bnk;
//
//          startaddr = BOOT_SECTOR1_OFS + (BOOT_FW_IMGMAX * (rcv_bank - 1));
...
//  cpyofs += ATP_IDENT_KEY_STG;
    ...
    if (p->bm == BOOT_MODE_UPDATE) {
        ...
        // (/EFI/BOOT/BOOTX64.efi || [prevmode == 1] ? previsouly bank : Factory bank )
